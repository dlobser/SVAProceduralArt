...
public class FlowParticlesAlongSplineWImage : MonoBehaviour {
	...
	public Vector4 noiseData = Vector4.zero;
	...
	void SetupEndTargets(){
		for (int i = 0; i < endTargetPos.Length; i++) {
			Vector3 c = FindPixel ();
//			Debug.Log (c);
			endTargetPos [i] = new Vector3 (c.x, c.y, Random.value*.2f);
			particleSize [i] = c.z;
		}
	}

	bool ColorDistance(Color A, Color B, float dist){
		Vector3 a = new Vector3 (A.r, A.g, A.b);
		Vector3 b = new Vector3 (B.r, B.g, B.b);
		//Debug.Log (a + " , " + b + " , " + Vector3.Distance (a,b));
		return Vector3.Distance (new Vector3 (A.r, A.g, A.b), new Vector3 (B.r, B.g, B.b)) < dist;
	}
	...
	Vector3 FindPixel(){
		...
		while (counter > 0 && b.Equals(Color.black)) {
			...
			//checkDistance = ColorDistance(b,imageColor,.1f);
//			if (checkDistance)
//				Debug.Log ("Awesome");
			r = new Vector3 ((((float)R.x / (float)image.width) - .5f), (((float)R.y / (float)image.height) - .5f),b.r);
			counter--;
		}
		return r;
	}
